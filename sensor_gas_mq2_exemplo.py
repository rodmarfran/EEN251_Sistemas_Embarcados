...
# Tempo de debounce para o sensor de gases inflamáveis MQ-2 em ms
debounce_time_ms = 10

# Variável global para armazenar o estado atual do sensor de gases inflamáveis MQ-2
gas_state = 0
# Variável global para armazenar o estado anterior do sensor de gases inflamáveis MQ-2
gas_last_state = 0

# Habilita o uso de interrupção para o sensor de gases inflamáveis MQ-2
enable_irq = True

# Configura o pino da saída digital do sensor de gases inflamáveis MQ-2 como entrada
gas_digital = Pin(gas_digital_pin, Pin.IN)

# Instancia o objeto ADC para leitura da saída analógica do sensor de gases inflamáveis MQ-2
gas_analog = ADC(Pin(gas_analog_pin))

# Instancia um objeto Timer para criar um temporizador para leitura da saída analógica do sensor de gases inflamáveis MQ-2
timer = Timer()

# Função temporizada para leitura da saída analógica do sensor de gases inflamáveis MQ-2 
def gas_analog_read(timer):
    # Lê o valor da entrada analógica do sensor de gases inflamáveis MQ-2 como um valor positivo de 16 bits
...
if enable_irq:
    # Função de callback para a interrupção do sensor de gases inflamáveis MQ-2
    def gas_callback(pin):
        # Atualiza a variável global gas_state com o valor do pino
        global gas_state
        gas_state = pin.value()

    # Adiciona a interrupção para detectar a mudança no estado do sensor de gases inflamáveis MQ-2 (borda de subida e descida)
    gas_digital.irq(trigger=Pin.IRQ_RISING | Pin.IRQ_FALLING, handler=gas_callback)
    
# Atualiza o estado atual e anterior do sensor de gases inflamáveis MQ-2
...
# Verifica o estado inicial do sensor de gases inflamáveis MQ-2
if gas_state == 1:
...
while True:
    ...
    if not enable_irq:
        # Lê o estado do sensor de gases inflamáveis MQ-2
        gas_state = gas_digital.value()
        
    # Verifica se o estado do sensor de gases inflamáveis MQ-2 mudou
    if gas_state != gas_last_state:
        # Aguarda um período para fazer o debounce do sensor de gases inflamáveis MQ-2
    ...
    # Verifica se ocorreu uma borda de subida no sinal do sensor de gases inflamáveis MQ-2
    if gas_state == 1 and gas_last_state == 0:
        # Atualiza o estado anterior do sensor de gases inflamáveis MQ-2
        gas_last_state = 1
        # Imprime que foram detectados gases inflamáveis pelo sensor de gases inflamáveis MQ-2
        print("Detectado de gases inflamáveis!")

    # Verifica se ocorreu uma borda de descida no sinal do sensor de gases inflamáveis MQ-2
    elif gas_state == 0 and gas_last_state == 1:
        # Atualiza o estado anterior do sensor de gases inflamáveis MQ-2
        gas_last_state = 0
        # Imprime que não foram detectados gases inflamáveis pelo sensor de gases inflamáveis MQ-2
        print("Não detectado gases inflamáveis!")
